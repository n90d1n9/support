...
class KnowledgeBaseNotifier extends StateNotifier<List<KbArticle>> {
  ...
  static List<KbArticle> _seed() {
    ...
    return [
      KbArticle(
        ...
        summary: 'Step-by-step guide for reviewing and resolving fare disputes.',
        body:
  ...
  List<KbArticle> getRelatedArticles(KbArticle article, {int limit = 3}) {
    ...
    related.sort((a, b) {
      final aMatches =
          a.tags.where((tag) => article.tags.contains(tag)).length;
      final bMatches =
          b.tags.where((tag) => article.tags.contains(tag)).length;
